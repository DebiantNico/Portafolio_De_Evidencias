...
pokemon.post("/", async(req, res, next) => {
    ...
    if (pok_name && pok_height && pok_weight && pok_base_experience) {
        ...
        if (rows.affectedRows == 1) {
            return res.status(201).json({ code: 201, message: "Pokemon insertado correctamenete."})
        }   
...
pokemon.put("/:id([0-9]{1,3})", async (req,res,next) => {
    ...
    if (pok_name && pok_height && pok_weight && pok_base_experience) {
        ...
        const rows = await db.query(query);
        if (rows.affectedRows == 1) {
            return res.status(200).json({ code: 200, message: "Pokemon actualizado correctamenete."})
        }   
...
// Patch solo se utilizará cuando vamos a cambiar uno o más datos, pero no todos los datos del registro
...
pokemon.get('/:id([0-9]{1,3})', async (req, res, next) => {
    ...
    return res.status(200).json.apply({ code: 200, message: pkmn});
    }
